	...
	public void listarClientesPorNome(String nome) {
		...
		for(Cliente p:lista) {
			...
			if (p.getNome().toLowerCase().contains(nome.toLowerCase())) { //transforma o nome em letra minuscula e verifica se contem o nome digitado 
				dados.addRow(new Object[] {								  // e o nome digitado tbm é transformado em letra minuscula	
						p.getCodigo(),									  //para poder achar todos os nomes independente se é maiusculo ou minusculo				
						p.getNome(),									  	
						p.getEmail(),
						p.getCpf(),
						p.getRg(),
